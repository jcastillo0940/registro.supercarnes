...
<div class="mb-3">
    ...
    <select name="tipo_evento" class="form-control" required>
        @foreach(['general' => 'General', 'rock_fest' => 'Rock Fest', 'bbq_challenge' => 'BBQ Challenge'] as $value => $label)
            <option value="{{ $value }}" @selected(old('tipo_evento', $event->tipo_evento ?? 'general') === $value)>{{ $label }}</option>
        @endforeach
    </select>
</div>
...
<div class="mb-3">
    <label class="inline-flex items-center gap-2">
        <input type="checkbox" name="public_voting_enabled" value="1" @checked(old('public_voting_enabled', $event->public_voting_enabled ?? false))>
        <span>Habilitar votación pública</span>
    </label>
</div>
