...
class AdminController extends Controller
{
    ...
    {
        $judgeScores = DB::table('evaluaciones')->select('fonda_id', DB::raw('AVG(puntaje) as judge_avg'))->groupBy('fonda_id');
        ...
        return $this->reactPage('admin.dashboard', compact('fondas'));
    }

    public function participantes()
    {
        $participantes = Participant::all();
        return $this->reactPage('admin.participantes', compact('participantes'));
    }

    public function usuarios()
    {
        $usuarios = User::all();
        return $this->reactPage('admin.usuarios', compact('usuarios'));
    }

    public function logistica()
    {
        $participantes = Participant::orderBy('orden_visita', 'asc')->get();
        return $this->reactPage('admin.logistica', compact('participantes'));
    }

    public function guardarOrden(Request $request)
    {
        $validated = $request->validate(['orden' => 'required|array', 'orden.*' => 'required|integer|min:1']);
    ...
    {
        $validated = $request->validate(['ajuste' => ['required', 'numeric', 'between:-100,100']]);
    ...
    public function descargarPDF()
    {
        ...
        $html = '<h2>Lista de logística</h2><table border="1" cellspacing="0" cellpadding="4"><tr><th>#</th><th>Participante</th><th>Orden</th></tr>';
        foreach ($participantes as $i => $p) {
            $html .= '<tr><td>' . ($i + 1) . '</td><td>' . e($p->nombre_fonda) . '</td><td>' . e($p->orden_visita) . '</td></tr>';
        }
        $html .= '</table>';

        return Pdf::loadHTML($html)->download('Lista_Oficial_Fonda_Challenge_2026.pdf');
    }
}
